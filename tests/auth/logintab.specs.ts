...
// Helper function to check if current project is tablet
function isTablet(projectName: string | undefined): boolean {
  return projectName?.startsWith('tablet-') ?? false;
}

test.describe('Login Page - UI Verification', () => {
  test.beforeEach(async ({ page }, testInfo) => {
    if (!isTablet(testInfo.project.name)) {
      test.skip(true, 'Tablet only test');
    }
  ...
  test('Page has correct title', async ({ page }) => {
    const loginPage = new LoginPage(page);
    await loginPage.verifyPageTitle(/RCTI/);
  });

  test('Login card/container is visible', async ({ page }) => {
    const loginPage = new LoginPage(page);
    await loginPage.verifyLoginCardVisible();
  });
